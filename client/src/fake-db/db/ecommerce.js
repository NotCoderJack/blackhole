...
MockInterceptor.onGet("/api/ecommerce/get-product-list").reply(config => {
  const response = EcommerceDB.productList;
  return [200, response];
});

MockInterceptor.onGet("/api/ecommerce/get-category-list").reply(config => {
  const response = EcommerceDB.category;
  return [200, response];
});

MockInterceptor.onGet("/api/ecommerce/get-rating-list").reply(config => {
  const response = EcommerceDB.rating;
  return [200, response];
});

MockInterceptor.onGet("/api/ecommerce/get-brand-list").reply(config => {
  const response = EcommerceDB.brand;
  return [200, response];
});

MockInterceptor.onGet("/api/ecommerce/get-cart-list").reply(config => {
  let uid = config.data;
  let response = [];
...
MockInterceptor.onPost("/api/ecommerce/add-to-cart").reply(config => {
...
MockInterceptor.onPost("/api/ecommerce/delete-from-cart").reply(config => {
  let { uid, productId } = JSON.parse(config.data);
...
MockInterceptor.onPost("/api/ecommerce/update-cart-amount").reply(config => {
