...
MockInterceptor.onGet("/api/inbox/all").reply(config => {
  return [200, inboxDB.messages];
});

MockInterceptor.onGet("/api/inbox").reply(config => {
  const id = config.data;
  const response = inboxDB.messages.find(message => message.id === id);
  return [200, response];
});

MockInterceptor.onPost("/api/inbox/delete").reply(config => {
...
MockInterceptor.onPost("/api/inbox/update").reply(config => {
  let message = JSON.parse(config.data);
  let index = { i: 0 };
  inboxDB.messages.forEach(element => {
    if (element.id === message.id) {
...
MockInterceptor.onPost("/api/inbox/add").reply(config => {
  let message = JSON.parse(config.data);
