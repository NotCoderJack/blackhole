...
MockInterceptor.onGet('/api/user/all').reply(config => {
    return [200, userDB.users]
});

MockInterceptor.onGet('/api/user').reply((config) => {
    const id = config.data;
    const response = userDB.users.find((user) => user.id === id);
    return [200, response];
});

MockInterceptor.onPost('/api/user/delete').reply(config => {
...
MockInterceptor.onPost('/api/user/update').reply((config) => {
    let user = JSON.parse(config.data);
    let index = { i: 0 };
    userDB.users.forEach(element => {
        if (element.id === user.id) {
...
MockInterceptor.onPost('/api/user/add').reply((config) => {
    let user = JSON.parse(config.data);
