...
MockInterceptor.onGet("/api/todo/all").reply(config => {
  const response = TodoDB.todo;
  return [200, response];
});

MockInterceptor.onGet("/api/todo/tag").reply(config => {
  const response = TodoDB.tag;
  return [200, response];
});

MockInterceptor.onGet("/api/todo").reply(config => {
...
MockInterceptor.onPost("/api/todo/reorder").reply(config => {
...
MockInterceptor.onPost("/api/todo/add").reply(config => {
...
MockInterceptor.onPost("/api/todo/tag/add").reply(config => {
...
MockInterceptor.onPost("/api/todo/update").reply(config => {
  let { todo } = JSON.parse(config.data);
...
MockInterceptor.onPost("/api/todo/delete").reply(config => {
  let { todo } = JSON.parse(config.data);
...
MockInterceptor.onPost("/api/todo/tag/delete").reply(config => {
  let { tag } = JSON.parse(config.data);
...
// MockInterceptor.onPost("/api/chat/create-chat-room").reply(config => {
