...
MockInterceptor.onGet("/api/calendar/events/all").reply(config => {
  return [200, calendarEventDB.events];
});

MockInterceptor.onPost("/api/calendar/events/add").reply(config => {
...
MockInterceptor.onPost("/api/calendar/events/update").reply(config => {
...
MockInterceptor.onPost("/api/calendar/events/delete").reply(config => {
