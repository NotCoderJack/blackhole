...
MockInterceptor.onGet("/api/chat").reply(config => {
...
MockInterceptor.onGet("/api/chat/chat-room").reply(config => {
...
MockInterceptor.onGet("/api/chat/contacts/recent").reply(config => {
  let temp = { contact: "" };
  const id = config.data;
...
MockInterceptor.onGet("/api/chat/contacts/all").reply(config => {
  const id = config.data;
  const response = ChatDB.contacts.filter(contact => contact.id !== id);
  return [200, response];
});

MockInterceptor.onGet("/api/chat/contacts").reply(config => {
  const id = config.data;
  const response = ChatDB.contacts.find(contact => contact.id === id);
  return [200, response];
});

MockInterceptor.onPost("/api/chat/add").reply(config => {
