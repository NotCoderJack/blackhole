...
MockInterceptor.onGet("/api/notification").reply(config => {
  const response = NotificationDB.list;
  return [200, []];
});

MockInterceptor.onPost("/api/notification/add").reply(config => {
  const response = NotificationDB.list;
  return [200, response];
});

MockInterceptor.onPost("/api/notification/delete").reply(config => {
...
MockInterceptor.onPost("/api/notification/delete-all").reply(config => {
  NotificationDB.list = [];
  const response = NotificationDB.list;
  return [200, response];
});
