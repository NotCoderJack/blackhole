...
MockInterceptor.onGet("/api/invoices/all").reply(config => {
  return [200, invoiceDB.invoices];
});

MockInterceptor.onGet("/api/invoices").reply(config => {
  const id = config.data;
  const response = invoiceDB.invoices.find(invoice => invoice.id === id);
  return [200, response];
});

MockInterceptor.onPost("/api/invoices/delete").reply(config => {
...
MockInterceptor.onPost("/api/invoices/update").reply(config => {
  let invoice = JSON.parse(config.data);
  let index = { i: 0 };
  invoiceDB.invoices.forEach(element => {
    if (element.id === invoice.id) {
...
MockInterceptor.onPost("/api/invoices/add").reply(config => {
  let invoice = JSON.parse(config.data);
